...
// TODO: Don't forget to ditch `format` coercion in engines for doing it in `spritesmith`
Canvas.supportedExtensions = ['jpeg', 'png', 'gif'];
Canvas.prototype = {
  ...
  'export': function (options, cb) {
    ...
    assert(format, '`options.format` wasn\'t provided to `pixelsmith`. Please provide a format to export');
    assert(Canvas.supportedExtensions.indexOf(format) !== -1,
    ...
    savePixels(ndarray, format).pipe(concat(function concatenateImage (buff) {
